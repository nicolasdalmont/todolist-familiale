...
import {
  createGardenCategoryAction,
  deleteGardenCategoryAction,
  moveGardenCategoryAction,
  updateGardenCategoryAction,
} from "@/lib/garden-category-actions";
import { CATEGORY_ICON_CHOICES, categoryIcon, categoryIconColor } from "@/lib/categories";
...
export function GardenCategoryManager({ categories }: { categories: Category[] }) {
  ...
  function handleCreate(e: FormEvent) {
    ...
    fd.set("icon", newIcon);
    startTransition(async () => {
      const res = await createGardenCategoryAction(fd);
      if (res.error) {
  ...
  function handleEdit(e: FormEvent, c: Category) {
    ...
    setEditError(null);
    startTransition(async () => {
      const res = await updateGardenCategoryAction(c.slug, { label: editLabel, icon: editIcon });
      if (res.error) {
    ...
    <div className="flex flex-col gap-3">
      ...
      {showCreate ? (
        <form onSubmit={handleCreate} className="flex flex-col gap-3 rounded-2xl border border-line bg-surface p-4 shadow-sm">
          <div>
            <label className="mb-1 block text-[12.5px] font-bold" htmlFor="newGardenCatLabel">
              Nom
            </label>
            <input
              id="newGardenCatLabel"
              type="text"
              required
              autoFocus
              value={newLabel}
      ...
      <div className="flex flex-col gap-2">
        {categories.map((c, i) => {
          ...
          const isFallback = c.slug === FALLBACK_GARDEN_CATEGORY_SLUG;
          return (
            <div key={c.slug} className="rounded-2xl border border-line bg-surface p-3.5 shadow-sm">
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="flex shrink-0 items-center gap-1">
                  <button
                    ...
                    aria-label="Monter"
                    onClick={() => run(() => moveGardenCategoryAction(c.slug, "up"), "Ordre mis à jour")}
                  ...
                  <button
                    ...
                    onClick={() => run(() => moveGardenCategoryAction(c.slug, "down"), "Ordre mis à jour")}
                    className="flex h-7 w-7 items-center justify-center rounded-lg text-ink-muted hover:bg-sand disabled:opacity-30"
                  >
                    <IconArrowLeft className="h-3.5 w-3.5 -rotate-90" />
                  </button>
              ...
              {editingSlug === c.slug ? (
                ...
                >
                  <div>
                    <label className="mb-1 block text-[12px] font-bold" htmlFor={`garden-cat-label-${c.slug}`}>
                      Nom
                    </label>
                    <input
                      id={`garden-cat-label-${c.slug}`}
                      type="text"
                      required
                      value={editLabel}
      ...
      <ConfirmDialog
        ...
        body="Les activités qui l'utilisent repasseront dans « Autre »."
        ...
        onConfirm={() => {
          ...
          setDeleting(null);
          if (target) run(() => deleteGardenCategoryAction(target.slug), "Catégorie supprimée");
        }}
      />
    </div>
  );
