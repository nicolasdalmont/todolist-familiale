...
// Onglet Santé (voir migration 007 et src/lib/health.ts) : liste des
// activités récurrentes de santé (visite médicale, dentiste, vaccin…).
// Même montage que Voiture (VoitureScreen.tsx) : chaque activité EST une
...
function activityToFormState(activity: HealthActivity): FormState {
  return {
    name: activity.name,
...
function ActivityForm({
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-3 rounded-2xl border border-line bg-surface p-4 shadow-sm">
      <div>
        <label className="mb-1 block text-[12.5px] font-bold" htmlFor="healthName">
          Nom
        </label>
        <input
          id="healthName"
          ...
          placeholder="Ex : Visite médicale, Dentiste…"
          className="w-full rounded-xl border border-line px-3 py-2.5 text-[14px] outline-none focus:border-brand"
        />
      </div>
      <div>
        <label className="mb-1 block text-[12.5px] font-bold" htmlFor="healthDescription">
          Description
        </label>
        <textarea
...
function matchesQuery(activity: HealthActivity, query: string): boolean {
...
export function SanteScreen({
  ...
  const [deleting, setDeleting] = useState<HealthActivity | null>(null);
  ...
  useEffect(() => {
    ...
    document.getElementById(`health-activity-${firstId}`)?.scrollIntoView({ block: "start" });
  }, [firstId]);
  ...
  function handleCreate(state: FormState) {
    setCreateError(null);
    startTransition(async () => {
      const res = await createHealthActivityAction(formStateToFormData(state));
      if (res.error) {
  ...
  function handleUpdate(activity: HealthActivity, state: FormState) {
    setEditError(null);
    const fd = formStateToFormData(state);
    fd.set("activityId", activity.id);
    startTransition(async () => {
      const res = await updateHealthActivityAction(fd);
      if (res.error) {
  ...
  function handleClose(activity: HealthActivity) {
    ...
    <div className="flex flex-col gap-4">
      ...
      {filtered.length === 0 ? (
        <p className="rounded-2xl border border-dashed border-line p-6 text-center text-[13px] text-ink-muted">
          {activities.length === 0 ? "Aucune activité santé pour l'instant." : "Aucune activité ne correspond à la recherche."}
        </p>
      ) : (
        <div className="flex flex-col gap-2">
          {filtered.map((activity) => {
            ...
            return (
              <div key={activity.id} id={`health-activity-${activity.id}`} className="scroll-mt-16 rounded-2xl border border-line bg-surface p-3.5 shadow-sm">
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          ...
          setDeleting(null);
          if (target) run(() => deleteHealthActivityAction(target.id), "Activité supprimée");
        }}
      />
    </div>
  );
