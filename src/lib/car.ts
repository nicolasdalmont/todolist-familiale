...
// Logique des activités récurrentes de la voiture (migration 005) — pas de
// "use server" ici : ce module n'expose aucune Server Action, il est
// importé aussi bien par src/lib/car-actions.ts (création/modification/
...
// est clôturée ou supprimée) — même montage que src/lib/garden.ts.
//
// Contrairement à Jardin (un ensemble de mois porté par une ligne
// persistante), chaque activité voiture EST une instance datée : sa
// clôture ne fait pas avancer un compteur sur la même ligne, elle crée une
// toute nouvelle ligne car_activities (voir advanceCarActivity ci-dessous).

export const CAR_CATEGORY_SLUG = "voiture";

// Échéance de la tâche générée : le jour exact à 9h Paris si connu, sinon
// le 1er du mois à 9h Paris (même convention que Jardin,
// src/lib/garden.ts::occurrenceDueAtIso).
...
export function nextCarDueDate(dueDate: string, recurrence: Recurrence): string | null {
  const next = computeNextOccurrence(`${dueDate}T00:00:00.000Z`, recurrence);
  return next ? next.slice(0, 10) : null;
}

type CarActivitySeed = {
  id: string;
  name: string;
  description: string;
  createdBy: string;
    ...
    assigneeIds
      ...
      .map((id) =>
        notifyUser({
          ...
          type: "task_shared",
          taskId: task.id,
          title: `Nouvelle tâche Voiture : « ${activity.name} »`,
        })
      )
  );
}
...
// est récurrente, crée l'instance suivante (nouvelle ligne car_activities
...
export async function advanceCarActivity(carActivityId: string): Promise<void> {
  ...
  await sql`update car_activities set status = 'done' where id = ${carActivityId}`;

  if (!activity.recurrence || activity.recurrence.type === "none") return;

  const next = nextCarDueDate(activity.due_date, activity.recurrence);
  if (!next) return;

  const assigneeRows = await sql`select user_id from car_activity_assignees where car_activity_id = ${carActivityId}`;
