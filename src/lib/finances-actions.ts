...
import { createFinancesOccurrenceTask, occurrenceDueAtIso } from "@/lib/finances";
import type { Recurrence, RecurrenceType } from "@/lib/types";

// Gestion des activités récurrentes de finances (onglet Finances, migration
// 008) — ouverte à tout utilisateur connecté, comme Jardin/Voiture/Santé.
// Même moule que src/lib/health-actions.ts (Server Actions "use server",
...
export async function createFinancesActivityAction(formData: FormData): Promise<Result> {
...
export async function updateFinancesActivityAction(formData: FormData): Promise<Result> {
  const userId = await getSessionUserId();
  if (!userId) redirect("/login");
  ...
  const existingRows = await sql`select id, created_by from finances_activities where id = ${activityId}`;
  const existing = existingRows[0] as { id: string; created_by: string } | undefined;
  if (!existing) return { error: "Activité introuvable." };

  try {
    await sql`
      update finances_activities
  ...
  await sql`delete from finances_activity_assignees where finances_activity_id = ${activityId}`;
  await sql`
    insert into finances_activity_assignees (finances_activity_id, user_id)
  ...
  // autorité (même logique que updateHealthActivityAction,
  // src/lib/health-actions.ts) — titre/description/échéance/responsables
  // sont systématiquement réécrits.
  const openTaskRows = await sql`
    select id from tasks where finances_activity_id = ${activityId} and status not in ('done', 'archived')
  `;
...
export async function deleteFinancesActivityAction(activityId: string): Promise<Result> {
  const userId = await getSessionUserId();
  if (!userId) redirect("/login");
  if (!activityId) return { error: "Activité introuvable." };
  ...
  // deleteHealthActivityAction, src/lib/health-actions.ts.
  const openTaskRows = await sql`
    select id from tasks where finances_activity_id = ${activityId} and status not in ('done', 'archived')
  `;
  const openTask = openTaskRows[0] as { id: string } | undefined;
  ...
  await sql`delete from finances_activities where id = ${activityId}`;
