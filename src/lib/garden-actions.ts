...
export async function createGardenActivityAction(formData: FormData): Promise<Result> {
  ...
  try {
    const rows = await sql`
      insert into garden_activities (name, description, months, created_by)
      values (${name}, ${description}, ${months}, ${userId})
      returning id
    `;
...
export async function updateGardenActivityAction(formData: FormData): Promise<Result> {
  ...
  try {
    await sql`update garden_activities set name = ${name}, description = ${description}, months = ${months} where id = ${activityId}`;
  } catch (e) {
