...
import { createHealthOccurrenceTask, occurrenceDueAtIso } from "@/lib/health";
import type { Recurrence, RecurrenceType } from "@/lib/types";

// Gestion des activités récurrentes de santé (onglet Santé, migration 007)
// — ouverte à tout utilisateur connecté, comme Jardin/Voiture. Même moule
// que src/lib/car-actions.ts (Server Actions "use server", FormData en
// entrée, résultat { error?, ok? }).
...
export async function createHealthActivityAction(formData: FormData): Promise<Result> {
...
export async function updateHealthActivityAction(formData: FormData): Promise<Result> {
  const userId = await getSessionUserId();
  if (!userId) redirect("/login");
  ...
  const existingRows = await sql`select id, created_by from health_activities where id = ${activityId}`;
  ...
  await sql`delete from health_activity_assignees where health_activity_id = ${activityId}`;
  await sql`
    insert into health_activity_assignees (health_activity_id, user_id)
  ...
  // autorité (même logique que updateCarActivityAction,
  // src/lib/car-actions.ts) — titre/description/échéance/responsables sont
  // systématiquement réécrits.
  const openTaskRows = await sql`
    select id from tasks where health_activity_id = ${activityId} and status not in ('done', 'archived')
  `;
...
export async function deleteHealthActivityAction(activityId: string): Promise<Result> {
  const userId = await getSessionUserId();
  if (!userId) redirect("/login");
  if (!activityId) return { error: "Activité introuvable." };
  ...
  // deleteCarActivityAction, src/lib/car-actions.ts.
  const openTaskRows = await sql`
    select id from tasks where health_activity_id = ${activityId} and status not in ('done', 'archived')
  `;
  const openTask = openTaskRows[0] as { id: string } | undefined;
  ...
  await sql`delete from health_activities where id = ${activityId}`;
