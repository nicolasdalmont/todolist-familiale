...
// notification s'ils ne sont pas eux-mêmes le créateur. `checklistLabels`
...
// tâche ouverte).
export async function createHealthOccurrenceTask(
  activity: HealthActivitySeed,
  assigneeIds: string[],
...
// `checklistLabels` (22/09/2026) : la checklist de la tâche qui vient de se
// terminer, à fournir par l'appelant (et non relue ici) — deleteTaskAction
// supprime la tâche, donc sa checklist (on delete cascade), avant d'appeler
// cette fonction ; il n'y aurait donc plus rien à lire à ce stade.
export async function advanceHealthActivity(healthActivityId: string, checklistLabels: string[] = []): Promise<void> {
  const rows = await sql`
